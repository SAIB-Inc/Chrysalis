...
public sealed class MapConverter : ICborConverter
{
    public object Read(CborReader reader, CborOptions options)
    {
        ...
        List<KeyValuePair<object, object?>> entries = ReadKeyValuePairs(reader, options);
        return CreateMapInstance(entries, options);
    }
    ...
    private static List<KeyValuePair<object, object?>> ReadKeyValuePairs(CborReader reader, CborOptions options)
    {
    ...
    private static (Type KeyType, Type ValueType) GetGenericMapTypes(CborOptions options)
    {
    ...
    private static object CreateMapInstance(List<KeyValuePair<object, object?>> entries, CborOptions options)
    {
        (Type KeyType, Type ValueType) genericTypes = GetGenericMapTypes(options);
