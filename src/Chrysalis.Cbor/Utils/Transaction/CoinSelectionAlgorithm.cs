...
public class CoinSelectionAlgorithm : ICoinSelection
{
    ...
    public static CoinSelectionResult LargestFirstAlgorithm(List<TransactionOutput> self, ulong requestedAmount, int maxInputs = 20)
    {
        ...
        List<TransactionOutput> sortedUtxos = [.. self.OrderByDescending(u => u.Lovelace() ?? 0)];
        List<TransactionOutput> selectedUtxos = [];
        ulong totalSelected = 0;

        while (totalSelected < requestedAmount)
        {
            if (!sortedUtxos.Any())
                throw new InvalidOperationException("UTxO Balance Insufficient");

            // Remove from head and add to selected set
            TransactionOutput utxo = sortedUtxos[0];
            sortedUtxos.RemoveAt(0);

            // Skip UTXOs with null amount or zero value
            ulong lovelace = utxo.Lovelace() ?? 0;
            if (lovelace == 0) continue;

            selectedUtxos.Add(utxo);
            totalSelected += lovelace;

            if (selectedUtxos.Count > maxInputs)
                throw new InvalidOperationException("Maximum Input Count Exceeded");
        }

        ulong change = totalSelected - requestedAmount;
