...
internal static class ByteStringBuiltins
{
    ...
    internal static CekValue LengthOfByteString(ImmutableArray<CekValue> args) =>
        IntegerResult(UnwrapByteString(args[0]).Length);
    ...
    internal static CekValue EqualsByteString(ImmutableArray<CekValue> args) =>
        BoolResult(UnwrapByteString(args[0]).Span.SequenceEqual(UnwrapByteString(args[1]).Span));

    internal static CekValue LessThanByteString(ImmutableArray<CekValue> args) =>
        BoolResult(UnwrapByteString(args[0]).Span.SequenceCompareTo(UnwrapByteString(args[1]).Span) < 0);
