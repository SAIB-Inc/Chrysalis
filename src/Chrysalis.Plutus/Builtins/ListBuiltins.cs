...
internal static class ListBuiltins
{
    ...
    internal static CekValue NullList(ImmutableArray<CekValue> args) => BoolResult(UnwrapListConstant(args[0]).Values.IsEmpty);

    internal static CekValue ChooseList(ImmutableArray<CekValue> args) => UnwrapListConstant(args[0]).Values.IsEmpty ? args[1] : args[2];
