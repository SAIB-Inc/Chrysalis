...
internal static class BuiltinHelpers
{
    ...
    internal static BigInteger UnwrapInteger(CekValue val) => val is VConstant { Value: IntegerConstant i }
            ? i.Value
            : throw new EvaluationException(
                $"expected integer constant, got {DescribeValue(val)}");

    internal static ReadOnlyMemory<byte> UnwrapByteString(CekValue val) => val is VConstant { Value: ByteStringConstant bs }
            ? bs.Value
            : throw new EvaluationException(
                $"expected bytestring constant, got {DescribeValue(val)}");

    internal static string UnwrapString(CekValue val) => val is VConstant { Value: StringConstant s }
            ? s.Value
            : throw new EvaluationException(
                $"expected string constant, got {DescribeValue(val)}");

    internal static bool UnwrapBool(CekValue val) => val is VConstant { Value: BoolConstant b }
            ? b.Value
            : throw new EvaluationException(
                $"expected bool constant, got {DescribeValue(val)}");

    internal static PlutusData UnwrapData(CekValue val) => val is VConstant { Value: DataConstant d }
            ? d.Value
            : throw new EvaluationException(
    ...
    internal static ArrayConstant UnwrapArrayConstant(CekValue val) => val is VConstant { Value: ArrayConstant a }
            ? a
            : throw new EvaluationException(
                $"expected array constant, got {DescribeValue(val)}");

    internal static ReadOnlyMemory<byte> UnwrapG1(CekValue val) => val is VConstant { Value: Bls12381G1Constant g }
            ? g.Value
            : throw new EvaluationException(
                $"expected bls12_381_g1_element, got {DescribeValue(val)}");

    internal static ReadOnlyMemory<byte> UnwrapG2(CekValue val) => val is VConstant { Value: Bls12381G2Constant g }
            ? g.Value
            : throw new EvaluationException(
                $"expected bls12_381_g2_element, got {DescribeValue(val)}");

    internal static ReadOnlyMemory<byte> UnwrapMlResult(CekValue val) => val is VConstant { Value: Bls12381MlResultConstant m }
            ? m.Value
            : throw new EvaluationException(
    ...
    internal static CekValue G2Result(byte[] bytes) => new VConstant(new Bls12381G2Constant(bytes));

    internal static CekValue MlResult(byte[] bytes) => new VConstant(new Bls12381MlResultConstant(bytes));

    internal static LedgerValue UnwrapValue(CekValue val) => val is VConstant { Value: ValueConstant v }
            ? v.Value
            : throw new EvaluationException(
                $"expected value constant, got {DescribeValue(val)}");

    internal static CekValue ValueResult(LedgerValue v) => new VConstant(new ValueConstant(v));
