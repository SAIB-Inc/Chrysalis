...
{
    private static BuiltinCostModel Const(long cpu, long mem) => new(new ConstantCost(cpu), new ConstantCost(mem));

    private static readonly CostFunction DivCpu = new ConstAboveDiagonalCost(
        85848, 85848, 123203, 1716, 7305, 57, 549, -900);

    /// <summary>
    /// The latest on-chain PlutusV3 cost model (Conway, protocol v11), captured from preprod
    /// <c>/epochs/latest/parameters</c> → <c>cost_models_raw.PlutusV3</c> (350 ints, verified
    /// node-exact). Used by <c>Evaluator</c> as the default when no provider protocol-params cost
    /// model is supplied, so tx evaluation is node-accurate out of the box. To refresh, re-fetch
    /// that array and replace the literals below. (Distinct from <see cref="Create"/>, which stays
    /// the Plutus-conformance reference model.)
    /// </summary>
    internal static readonly long[] LatestV3 =
    [
