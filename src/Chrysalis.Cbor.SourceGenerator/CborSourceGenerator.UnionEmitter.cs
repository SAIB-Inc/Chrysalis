...
public sealed partial class CborSourceGenerator
{
    ...
    private class UnionEmitterStrategy : EmitterStrategyBase
    {
        ...
        public override string EmitDeserializer(SerializableType type)
        {
            ...
            helperMethods.AppendLine("                var keyState = reader.PeekState();");
            helperMethods.AppendLine("                ");
            helperMethods.AppendLine("                if (keyState == CborReaderState.TextString)");
            helperMethods.AppendLine("                {");
            helperMethods.AppendLine("                    key = reader.ReadTextString();");
            helperMethods.AppendLine("                }");
            helperMethods.AppendLine("                else if (keyState == CborReaderState.UnsignedInteger)");
            helperMethods.AppendLine("                {");
            helperMethods.AppendLine("                    key = reader.ReadUInt32().ToString();");
            helperMethods.AppendLine("                }");
            helperMethods.AppendLine("                else if (keyState == CborReaderState.NegativeInteger)");
            helperMethods.AppendLine("                {");
            helperMethods.AppendLine("                    key = reader.ReadInt32().ToString();");
            helperMethods.AppendLine("                }");
            helperMethods.AppendLine("                else");
            helperMethods.AppendLine("                {");
            helperMethods.AppendLine("                    // For any other key type, throw an exception");
            helperMethods.AppendLine("                    if (keyState == CborReaderState.ByteString)");
            helperMethods.AppendLine("                    {");
            helperMethods.AppendLine("                        byte[] bytes = reader.ReadByteString();");
            helperMethods.AppendLine("                        key = BitConverter.ToString(bytes).Replace(\"-\", \"\");");
            helperMethods.AppendLine("                    }");
            helperMethods.AppendLine("                    else if (keyState == CborReaderState.StartArray)");
            helperMethods.AppendLine("                    {");
            helperMethods.AppendLine("                        // For array keys, read the array and convert it to a string representation");
            helperMethods.AppendLine("                        var arrayKey = new List<object>();");
            helperMethods.AppendLine("                        reader.ReadStartArray();");
            helperMethods.AppendLine("                        while (reader.PeekState() != CborReaderState.EndArray)");
            helperMethods.AppendLine("                        {");
            helperMethods.AppendLine("                            arrayKey.Add(ReadGenericValue(reader));");
            helperMethods.AppendLine("                        }");
            helperMethods.AppendLine("                        reader.ReadEndArray();");
            helperMethods.AppendLine("                        key = $\"Array[{string.Join(\",\", arrayKey)}]\";");
            helperMethods.AppendLine("                    }");
            helperMethods.AppendLine("                    else if (keyState == CborReaderState.StartMap)");
            helperMethods.AppendLine("                    {");
            helperMethods.AppendLine("                        // For map keys, read the map and use its hash code");
            helperMethods.AppendLine("                        var mapValue = ReadGenericValue(reader);");
            helperMethods.AppendLine("                        key = $\"Map_{mapValue.GetHashCode()}\";");
            helperMethods.AppendLine("                    }");
            helperMethods.AppendLine("                    else");
            helperMethods.AppendLine("                    {");
            helperMethods.AppendLine("                        // Process all other types");
            helperMethods.AppendLine("                        reader.SkipValue();");
            helperMethods.AppendLine("                        key = $\"Unknown_{keyState}_{Guid.NewGuid().ToString(\"N\")}\";");
            helperMethods.AppendLine("                    }");
            helperMethods.AppendLine("                }");
            helperMethods.AppendLine("                ");
            ...
            helperMethods.AppendLine("            catch (Exception ex)");
            helperMethods.AppendLine("            {");
            helperMethods.AppendLine("                Console.WriteLine($\"Exception during array reading: {ex.Message}\");");
            helperMethods.AppendLine("                // Safely read end array or skip to end if needed");
            helperMethods.AppendLine("                try { reader.ReadEndArray(); } catch { /* Skip if this fails */ }");
            helperMethods.AppendLine("                return new List<object>();");
            helperMethods.AppendLine("            }");
