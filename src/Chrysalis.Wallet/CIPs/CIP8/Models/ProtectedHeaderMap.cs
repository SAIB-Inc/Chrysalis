...
public class ProtectedHeaderMap
{
    ...
    public ProtectedHeaderMap(byte[] serializedMap) => _serializedMap = serializedMap ?? [];

    /// <summary>
    /// Creates a protected header map from a HeaderMap.
    /// </summary>
    /// <param name="headerMap">The header map to serialize.</param>
    public ProtectedHeaderMap(HeaderMap headerMap) => _serializedMap = headerMap == null || headerMap == HeaderMap.Empty
            ? []
    ...
    public HeaderMap Deserialize() => _serializedMap.Length == 0 ? HeaderMap.Empty : CborSerializer.Deserialize<HeaderMap>(_serializedMap);
}
